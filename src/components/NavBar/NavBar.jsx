...
const NavBar = ({isVisible, setIsVisible, links, logo}) => {
    return (
        <nav className={`flex justify-between items-center md:mx-7 py-5 p-2 md:p-10 bg-white`}>
            <div className="z-[60]" onClick={() => setIsVisible(false)}>
                <Logo logo={logo}/>
            </div>
            <ul className={`${isVisible?'flex flex-col absolute top-0 p-10 left-[50%] translate-x-[-50%] z-50 bg-white w-full':'hidden'} md:flex justify-center items-center gap-5 min-[1120px]:gap-10 max-[767px]:shadow-sm shadow-neutral-400 duration-1000`}>
